...
For implementing proper logging and monitoring features for both real-time and batch data ingestions, the platform would need slight redesigning. One design choice would be to create new manager and monitor components for the batch data ingestion. Another option, would be to modify the streaming data manager and monitor components to handle also the batch ingestion. This choice would be less fragmanted and over-engineered. The manager would check for streaming data from the tenant topics similar to streaming manager, but it would also check for new batch data from the tenant staging input directory. When the manager finds new data, it will invoke the corresponding tenant's batch and/or streaming ingestion pipelines to perform based on the input by sending Kafka message. The logic for stopping the pipelines would stay the same, the manager would send corresponding Kafka message to the pipeline.

The monitor component would also work with the same logic as just for the streaming monitor. The monitor would receive ingestion status metrics from both streaming and batch ingestion pipelines by Kafka messages, and depending on the metrics, make calls to the manager to stop or restart pipeline component.

![architecture of logging and monitoring for full platform](../images/fullmonitoring_architecture.png)

The platform provider would know the ingestion statistics by reports generated by the monitor component. The monitor receives the ingestion metrics in real-time during the ingestion, and based on the input it could inform the platform provider the statuses by for example generating some visualization (e.g. powerBI). The monitor could also send alarm to platform provider via email, sms or local platform calls. The choice would depend on platform hosting, number of employees etc (do we have operations team monitoring the statistics continuosly?) The statistics the monitor would inform about would include amount of data ingested, existing errors (such as bad input data), performance (rows inserted/s etc.). Based on the ingestion statistics, the platform provider could take actions, such as manual component restarting, bug fixing and so on.

### 3.2 Stream ingestion to multiple data sinks

If the platform would provide options for tenants to store data into multiple data sinks, the platform would need slight modifications. We would create new data storage components, with technology choices depending on data format and data query needs. The tenant's streaming ingestion pipeline would insert the data into the other data storage as before. For example, if the other data storage would also be Cassandra and pipeline technology is Apache Flink, the insertion would just be with another Flink Cassandra sink developed. If the other data storage would be for analytical data, we could create a data warehouse component and develop the insertion mechanism corresponding to the technology.

![architecture with multiple data sinks](../images//architecture_multiplesinks.png)

### 3.3 Data encryption

The data to be ingested could be encrypted in several layers. The data could have some encrypted columns, such as personal identification data. The whole files could also be encrypted. This would mean that batch ingestion pipeline would have to deal with encrypted data. There are several mechanisms for processing encrypted data. The data could also be decrypted after ingesting from staging input directory, then processed and then encrypted again and stored to the data storage. The batch processing technology of this platform Apache Spark supports encryption. As the staging input directory is HDFS, Spark has native encryption mechanisms such as HDFS Transparent Encryption, which in implements an transparent end-to-end encryption for reading and writing data. This way the tenant's source data would be secured by encryption during the ingestion and transformation pipeline.

### 3.4 Quality of real-time ingestion data

If the platform would want to support detecting quality of data to ingest and transform and store the data quality detected, there would be several designs for implementing this. In the current implementation of streaming ingestion the data quality is detected by checking if the data contains values for data storage primary keys and transforming missing values to Cassandra-compatible Null-format. We could also validate the whole schema, to make sure that all the required fields are present. We could do data consistency checking, for ensuring numeric values are feasible and fall within expected ranges to filter out any anomalities. We could also implement some kind of duplication detection. The streaming ingestion techonoly of the platform Apache Flink supports several real-time validation techniques.

After measuring the data quality, the results would be stored in the platform for monitoring and analysis. In the current implementation, the number of accepted data rows and number of discarded data rows is stored as log files. The data would be stored to a database or other similar data sink. We could also store more information about the discarded rows, such as reasons for rejection. The reasons could include missing values, invalid formats, values between ranges and other anomalities. This kind of data would provide possibility for the platform to track data quality of tenants over time.
